...
SERVER="https://psychic-space-journey-4w6xv7xj4q6fq7gw-6443.app.github.dev"
# Exit on error
set -e
...
setup_configs() {
   ...
   # fi
    ...
    if [ ! -f "/tmp/token.csv" ]; then
        TOKEN="1234567890"
        echo "${TOKEN},kubelet-bootstrap,kubelet-bootstrap,system:bootstrappers" > /tmp/token.csv
    fi
   ...
   # openssl req -x509 -new -nodes -key /tmp/ca.key -subj "/CN=kubelet-ca" -days 365 -out /tmp/ca.crt
    ...
    if ! sudo kubebuilder/bin/kubectl config current-context | grep -q "test-context"; then
        ...
        sudo kubebuilder/bin/kubectl config set-cluster test-env --server=${SERVER} --insecure-skip-tls-verify
...
start() {
    ...
    sudo cp /root/.kube/config /var/lib/kubelet/bootstrap-kubeconfig
